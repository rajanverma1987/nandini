...
class Header extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.props.get_notification_status !==
        prevProps.get_notification_status &&
      this.props.get_notification_status === status.SUCCESS
    ) {
      if (
        this.props.get_notification_data &&
        this.props.get_notification_data.length > 0
      ) {
    ...
    if (
      this.props.get_company_status !== prevProps.get_company_status &&
      this.props.get_company_status == status.SUCCESS
    ) {
      if (
        this.props.get_company_status &&
        this.props.get_company_data &&
        this.props.get_company_data.Data
      ) {
  ...
  handleChange = (event, value) => {
    if (value) {
      const selectedOption = this.state.autoSelectList.find(
        (option) => option.RemoteCmpName === value
      );
      if (selectedOption) {
  ...
  render() {
    ...
    return (
      <>
        <div className="navbar-custom" style={{ backgroundColor: "red" }}>
                    ...
                    <div className="form-group-common ">
                      <Stack spacing={1} sx={{ width: 300 }}>
                        <Autocomplete
                          ...
                          id="free-solo-2-demo"
                          disableClearable
                          options={autoSelectList.map(
                            (option) => option.RemoteCmpName
                          )}
                          renderInput={(params) => (
                            <TextField
                              {...params}
                      ...
                      <ul>
                        ...
                        <li>
                          <span
                            ...
                            onClick={this.openLogOutModel}
                          >
                            <strong>{firstName}</strong> <br></br>Super Admin
                          </span>
                        </li>
