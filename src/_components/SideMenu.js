        ...
        >
          {!nav?.children ? (
            <ListItem
              ...
              tabIndex="0"
              component={Link != null && Link}
              to={nav?.to}
              onClick={() => this.setActiveTab(i, nav.to)}
            >
          ...
          {nav.children && openedSubMenus[i] && (
            <ul tabIndex="0">{this.displayChild(nav.children)}</ul>
          )}
        </li>
      );
    }
      ...
      childData.push(
        <ListItem key={data[j].name}>
          <Link
            ...
            onClick={() => this.handleTabs(data[j].to)}
          >
            {data[j].name}
          </Link>
        </ListItem>
      ...
      <>
        ...
        <div className={isOpen ? "sidebar open" : "sidebar"}>
          ...
          <SimpleBar style={{ maxHeight: "calc(100% - 76px)" }}>
            <List className="sidebar-content">{this.displaySidebarMenu()}</List>
          </SimpleBar>
        </div>
      </>
    );
...
const mapStateToProps = (state) => {
  const { get_inbox_status, inbox_data, tab_Data, tab_status } =
    state.procurement;
